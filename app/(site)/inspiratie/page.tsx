...
const inspiratieQuery = groq`
  *[_type == "article" && category == "inspiratie" && defined(publishedAt) && publishedAt <= now() && count(*[_type == "dossier" && references(^._id)]) == 0] | order(select(pinned == true => 0, 1), publishedAt desc) {
    _id,
    _type,
    title,
    "slug": slug.current,
